...
use super::{WorkerConfig, WorkerDetector, WorkerBootstrap, WorkerInfo, WorkerType};
use std::path::Path;
...
impl WorkerCompiler {
    ...
    pub fn compile_worker(&mut self, worker_info: &mut WorkerInfo) -> anyhow::Result<()> {
        ...
        let output = match worker_info.worker_type {
            WorkerType::Module => {
                self.compile_module_worker(&source, worker_info)?
            }
            WorkerType::Classic => {
                self.compile_classic_worker(&source, worker_info)?
            }
        };
    ...
    pub fn get_compiled(&self, name: &str) -> Option<&String> {
...
mod tests {
    ...
    fn test_worker_declaration_generation() {
        ...
        let workers = vec![
            WorkerInfo::new("test.worker.ts", WorkerType::Module),
        ];
