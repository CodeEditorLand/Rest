...
use super::{LocalizationBundle, LocalizationEntry, NLSConfig};
...
impl NLSBundle {
    ...
    pub fn get_bundle(&self, language: &str) -> Option<&LocalizationBundle> {
        self.bundles.get(language)
    }

    /// Get all bundles
    pub fn all_bundles(&self) -> &HashMap<String, LocalizationBundle> {
        &self.bundles
    }
    ...
    pub fn load_bundle(language: &str, path: &Path) -> std::io::Result<LocalizationBundle> {
        ...
        let bundle: LocalizationBundle = serde_json::from_str(&content)
