...
impl NLSExtractor {
    pub fn new() -> Self {
        Self {
            ...
            localize_pattern: Regex::new(r#"(?:nls\.)?localize\s*\(\s*['"]([^'"]+)['"]\s*,\s*['"]([^'"]*)['"]\s*\)"#).unwrap(),
            // Match nls.localize2('key', 'v1', 'v2')
            localize2_pattern: Regex::new(r#"(?:nls\.)?localize2\s*\(\s*['"]([^'"]+)['"]\s*,\s*['"]([^'"]*)['"]\s*,\s*['"]([^'"]*)['"]\s*\)"#).unwrap(),
        }
    }
    ...
    pub fn extract(&mut self, source: &str) {
        ...
        for cap in self.localize_pattern.captures_iter(source) {
            if let (Some(key), Some(value)) = (cap.get(1), cap.get(2)) {
                self.entries.entry(key.as_str().to_string())
