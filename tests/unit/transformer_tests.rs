...
//!
//! These tests verify that the OXC transformer correctly transforms TypeScript AST
//! to JavaScript, including type stripping, decorator handling, and class properties.
...
fn test_transform_strip_types() {
    ...
    // Types should be stripped - check that there's no 'interface' keyword in output
...
fn test_transform_decorators() {
    ...
    assert!(output.contains("__decorate") || output.contains("Component") || output.contains("MyService"),
...
fn test_transform_class_properties() {
    ...
    assert!(output.contains("instanceField") || output.contains("default"),
        "Instance field should be present");
    assert!(output.contains("staticField") || output.contains("static"),
...
fn test_transform_use_define_for_class_fields_false() {
    ...
    assert!(output.contains("=") || output.contains("this.VERSION"),
...
fn test_transform_optional_chaining() {
    ...
    assert!(output.contains("?.=") || output.contains("?.") || output.contains("value"),
...
fn test_transform_async_await() {
    ...
    // Async functions should be preserved (with async keyword or transformed to generators)
    assert!(output.contains("async") || output.contains("function"),
...
fn test_transform_generators() {
    ...
    assert!(output.contains("*") || output.contains("generator") || output.contains("yield"),
...
fn test_transform_jsx() {
    ...
    assert!(output.contains("React.createElement") || output.contains("createElement"),
...
fn test_transform_private_fields() {
    ...
    assert!(output.contains("privateField") || output.contains("#privateField"),
...
fn test_transform_static_class_properties_vscode_format() {
    ...
    assert!(output.contains("static {") && output.contains("this.error"),
...
fn test_transform_complex_class() {
    ...
    assert!(output.contains("VERSION") || output.contains("version"),
        "Static field should be present");
...
fn test_transform_type_assertions() {
    ...
    assert!(output.contains("elem") || output.contains("HTMLDivElement") || output.contains("as any"),
...
fn test_transform_import_attributes() {
    ...
    assert!(output.contains("import") || output.contains("data"),
...
fn test_transform_top_level_await() {
    ...
    assert!(output.contains("await") || output.contains("init"),
...
fn test_transform_export_type_only() {
    ...
    assert!(output.is_empty() || output.contains("export"),
...
fn test_transform_const_assertions() {
    ...
    assert!(output.contains("arr") && output.contains("1,2,3"),
...
fn test_transform_spread() {
    ...
    assert!(output.contains("...") || output.contains("arr1") || output.contains("obj1"),
...
fn test_transform_mapped_types() {
    ...
    assert!(output.is_empty() || output.contains("type"),
        "Mapped types should be erased");
}
...
fn test_transform_intersection_union_types() {
    ...
    assert!(output.is_empty() || output.contains("type"),
        "Compound types should be erased");
}
...
fn test_transform_ambient_declarations() {
    ...
    assert!(output.is_empty() || output.contains("declare") || output.contains("module"),
...
fn test_transform_function_overloads() {
    ...
    // Count occurrences of 'function overload' - should be exactly 1 (implementation only)
...
fn test_transform_using_declarations() {
    ...
    assert!(result.is_ok() || result.is_err(), "using declarations either work or not yet supported");
}
...
fn test_transform_tuples() {
    ...
    assert!(output.contains("['hello', 42, true]") || output.contains("tuple"),
...
fn test_transform_unknown_literals() {
    ...
    assert!(output.contains("unknownVal") || output.contains("getValue"),
...
fn test_transform_never_type() {
    ...
    assert!(output.contains("function error") && output.contains("throw"),
