...
impl CompatibilityReport {
    pub fn success(&self) -> bool {
        self.overall_match && self.mismatched_files.is_empty() && self.missing_in_rest.is_empty() && self.extra_in_rest.is_empty()
    }
}
...
mod tests {
    ...
    /// Full integration test comparing Rest output to VSCode out-build/ directory
    #[test]
    #[ignore] // Requires VSCode build to be present
    ...
    fn compare_vscode_output(
        config: &VSCodeTestConfig,
    ) -> Result<CompatibilityReport, Box<dyn std::error::Error>> {
        info!(
            "Comparing Rest output vs VSCode output: {}",
            config.vscode_source.display()
        );
        ...
        for ts_file in &ts_files {
            let rel_path = ts_file.strip_prefix(&config.vscode_source).unwrap();
            let vscode_js_path = config
                .vscode_output
                .join(rel_path)
                .with_extension("js");

            let rest_js_path = config
                .rest_output
                .join(rel_path)
                .with_extension("js");
        ...
        report.overall_match = report.mismatched_files.is_empty()
            && report.missing_in_rest.is_empty()
            && report.extra_in_rest.is_empty();
